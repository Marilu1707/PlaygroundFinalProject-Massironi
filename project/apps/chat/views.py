...
from chat.forms import FormularioIniciarConversacion, FormularioEnvioMensaje
...
# Búsqueda de TODAS las conversaciones recibidas e iniciadas por el usuario
    ...
    return render(request, 'chat/ver_chats.html', { 'chats' : chats })
...
# Disponibiliza SÓLO aquellos usuarios con los que puedo iniciar una conversación
    ...
    return render(request, 'chat/iniciar_conversacion.html', { 'formulario_iniciar_chat' : formulario_iniciar_chat })
...
# Esta vista disponibiliza la conversación con el usuario, y permite enviar un nuevo mensaje
    ...
    return render(request, 'chat/ver_chat.html', { 'formulario_envio_mensaje' : formulario_envio_mensaje, 'chat' : chat, 'mensajes' : mensajes })
...
class EliminarChatView(LoginRequiredMixin, DeleteView):
    model         = Chat
    template_name = 'chat/borrar_chat.html'
    success_url   = '/chats/'
